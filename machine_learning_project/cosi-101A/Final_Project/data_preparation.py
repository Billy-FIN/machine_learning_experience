...
if __name__ == "__main__":
    ...
    dt_original_21.head()
    dt_original_22.head()
    dataset1 = pd.concat([dt_original_17, dt_original_18, dt_original_19, dt_original_20, dt_original_21, dt_original_22], ignore_index=True)                  # get the values from the csv file
    ...
    for i in range(110301):
        tmp = age[i]
        age_p[i] = tmp * 12 - 192

    dataset1.iloc[:, 9] = age_p[:, 0].astype(float)
    
    dataset2 = np.zeros((110301, 51))            # create a second dataset for storing the processed data
    dataset2[:, 1:15] = dataset1.iloc[:, [0, 4, 6, 9, 11, 12, 13, 17, 22, 27, 28, 29, 30, 31]].values.astype(int)
    dataset2[:, 1] = dataset2[:, 1].astype(int)
    dataset2[:, 15:49] = dataset1.iloc[:, 43:77].values.astype(int)
    dataset2[:, [49, 50]] = dataset1.iloc[:, [5, 7]].values.astype(int)
    ...
    sorting_key = dataset2[:, 1].astype(int) * 100 + dataset2[:, 0].astype(int)
    ...
    test_data = pd.DataFrame(dataset3)
    test_data.to_csv(test_data_path, index=False)
    print('train_data saved at: ', test_data_path)
